...
aqi= pd.read_csv('./AQI-n5.csv' ,index_col=0,encoding="big5")
aqi.dropna(how='any',inplace=True)

aqi['AQI1']=aqi['AQI1'].astype(np.float32)
aqi['AQI2']=aqi['AQI2'].astype(np.float32)
aqi['AQI3']=aqi['AQI3'].astype(np.float32)
aqi['AQI4']=aqi['AQI4'].astype(np.float32)
aqi['AQI5']=aqi['AQI5'].astype(np.float32)
...
def normalize(df):
    ...
    newdf['AQI1'] = min_max_scaler.fit_transform(df['AQI1'].values.reshape(-1,1))
    newdf['AQI2'] = min_max_scaler.fit_transform(df['AQI2'].values.reshape(-1,1))
    newdf['AQI3'] = min_max_scaler.fit_transform(df['AQI3'].values.reshape(-1,1))
    newdf['AQI4'] = min_max_scaler.fit_transform(df['AQI4'].values.reshape(-1,1))
    newdf['AQI5'] = min_max_scaler.fit_transform(df['AQI5'].values.reshape(-1,1))
...
def denormalize(df, norm_value):
    original_value = df['AQI'].values.reshape(-1,1)
